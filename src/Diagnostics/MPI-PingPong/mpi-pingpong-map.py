...
def main():
    ...
    commandGenerateOutput = " && cat data | head -n1 >output && cat data | tail -n1 >>output && cat timeResult >>output && cat raw >>output && cat output"
    commandGenerateError = " || (errorcode=$? && echo -1 >error && echo -1 >>error && cat timeResult >>error && cat stdout stderr >>error && cat error && exit $errorcode)"
    commandLine = "TIMEFORMAT='%3R' && (time timeout [timeout]s bash -c '[sshcommand] && source {}/impi/`ls /opt/intel/impi`/bin64/mpivars.sh && [mpicommand]' >stdout 2>stderr) 2>timeResult".format(intelMpiLocation) + commandParseResult + commandGenerateOutput + commandGenerateError
