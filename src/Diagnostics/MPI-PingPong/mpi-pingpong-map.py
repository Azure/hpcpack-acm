...
def main():
    ...
    commandGenerateErrorAsJson = (" || (errorcode=$?"
                                  ...
                                  " && cat json"
                                  " && exit $errorcode)")
    commandLine = "TIMEFORMAT='%3R' && (time timeout [timeout]s bash -c '[sshcommand] && source {}/impi/`ls /opt/intel/impi`/bin64/mpivars.sh && [mpicommand]' >stdout 2>stderr) 2>timeResult".format(intelMpiLocation) + commandParseResult + commandGenerateOutputAsJson + commandGenerateErrorAsJson
