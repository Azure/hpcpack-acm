...
def main():
    ...
    commandLine = "source {0}/impi/`ls {0}/impi | head -n1`/bin64/mpivars.sh && [mpicommand] 2>stderr | tail -n29 | head -n25 || (errorcode=$? && cat stderr && exit $errorcode)".format(intelMpiLocation)
