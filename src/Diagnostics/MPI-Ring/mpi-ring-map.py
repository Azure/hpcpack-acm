...
def main():
    ...
    commandLine = "source {0}/impi/`ls {0}/impi | head -n1`/bin64/mpivars.sh && [mpicommand] >stdout 2>stderr && cat stdout | tail -n29 | head -n25 || (errorcode=$? && cat stdout stderr && exit $errorcode)".format(intelMpiLocation)
